...
function setupComparison() {
	...
	//d3.select(".proGolfEvent").text("Nombre de participations au " + proGolfEvents[minLocation]); // removed from display
	...
	maxRadarValue = Math.max(d3.max(radarData[1], function(d){return Math.abs(d.value); }), d3.max(radarData[2], function(d){return Math.abs(d.value); })) ;
	////////////////////////////////////////////////////////////// 
	...
	var marginModal = {top: 30, right: 70, bottom: 30, left: 70},
		widthModal = 320 - marginModal.left - marginModal.right,
		heightModal = widthModal,
		radarProClass = [".andrewAce",".brianBogey",".charlesCourse",".deborahDrive",".emmaEagle",".florenceFore"];
	
	//1.16 is the maximum z-score of any pro
	var maxRadarValuePro = Math.max(0.9, d3.max(golferZscore)) ;
		...
		setTimeout(function() {
			for (var i = 0; i < pro.length; i++) {
				...
				var radarDataPro = [
					...
					[//Pro
						{axis:'Vitesse de la balle', value: pro[i].values[1].zScore},
						{axis:'Vitesse du club', value: pro[i].values[0].zScore},
						{axis:'Carry', value: pro[i].values[3].zScore},
						{axis:'Side', value: -1*pro[i].values[4].zScore},
						{axis:'Angle d attaque', value: pro[i].values[2].zScore}
					],
					[//Golfer
						{axis:'Vitesse de la balle', value: golferZscore[1]},
						{axis:'Vitesse du club', value: golferZscore[0]},
						{axis:'Carry', value: golferZscore[3]},
						{axis:'Side', value: -1*golferZscore[4]},
						{axis:'Angle d attaque', value: golferZscore[2]}
